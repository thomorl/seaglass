 ...
 *
 */
public class SeaGlassTreeUI extends BasicTreeUI implements PropertyChangeListener, SeaglassUI {
    private SynthStyle   style;
    private SynthStyle   cellStyle;

    private SeaGlassContext paintContext;
    ...
    public SeaGlassContext getContext(JComponent c) {
        return getContext(c, SeaGlassLookAndFeel.getComponentState(c));
    }
    ...
    private SeaGlassContext getContext(JComponent c, Region region, int state) {
        return SeaGlassContext.getContext(SeaGlassContext.class, c, region, cellStyle, state);
    }
    ...
    public void paintBorder(SynthContext context, Graphics g, int x, int y, int w, int h) {
        ((SeaGlassContext)context).getPainter().paintTreeBorder(context, g, x, y, w, h);
    }
    ...
    protected void paint(SynthContext context, Graphics g) {
        paintContext = (SeaGlassContext)context;
        ...
        SeaGlassContext cellContext = getContext(tree, Region.TREE_CELL);
        ...
        if (paintingEnumerator != null) {
            ...
            while (!done && paintingEnumerator.hasMoreElements()) {
                ...
                if (path != null) {
                    ...
                    paintRow(renderer, dtcr, paintContext, cellContext, g, paintBounds, insets, bounds, rowBounds, path, row, isExpanded,
    ...
    protected void paintHorizontalLine(Graphics g, JComponent c, int y, int left, int right) {
        paintContext.getStyle().getGraphicsUtils(paintContext)
            .drawLine(paintContext, "Tree.horizontalLine", g, left, y, right, y, linesStyle);
    }
    ...
    protected void paintVerticalLine(Graphics g, JComponent c, int x, int top, int bottom) {
        paintContext.getStyle().getGraphicsUtils(paintContext)
            .drawLine(paintContext, "Tree.verticalLine", g, x, top, x, bottom, linesStyle);
    }
    ...
    private class SynthTreeCellRenderer extends DefaultTreeCellRenderer implements UIResource {
        ...
        public void paint(Graphics g) {
            paintComponent(g);
            if (hasFocus) {
                SeaGlassContext context = getContext(tree, Region.TREE_CELL);
                ...
                if (getComponentOrientation().isLeftToRight()) {
                    context.getPainter().paintTreeCellFocus(context, g, imageOffset, 0, getWidth() - imageOffset, getHeight());
                } else {
                    context.getPainter().paintTreeCellFocus(context, g, 0, 0, getWidth() - imageOffset, getHeight());
                }
                context.dispose();
            }
            SeaGlassLookAndFeel.resetSelectedUI();
    ...
    //
    private class ExpandedIconWrapper extends SeaGlassIcon {
        public ExpandedIconWrapper() {
            super("Tree", "expandedIconPainter", 7, 7);
        }
